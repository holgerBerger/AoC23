...
short[dchar] cardmap = [ 'A': 14, 'K': 13, 'Q':12, 'T':10, '9': 9, '8':8, '7':7, '6':6, '5':5, '4':4, '3':3, '2':2, 'J':1 ];
...
Handtype gettype(Hand hand) {
    ...
    foreach(c; hand) {
        if (c == 1) jokers++;
            ...
            if (c in counter) counter[c]++; else counter[c]=1;
    }
    debug stdout.write(counter, " J=", jokers);
...
Handsbid parse(const char[] line) {
    ...
    auto t = gettype(hand);
    debug stdout.writeln(" -> ", t);
    return Handsbid(hand, t, to!int(sp[1]));
